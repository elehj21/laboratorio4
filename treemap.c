...
Pair * searchTreeMap(TreeMap * tree, void* key) {
    ...
    while(aux != NULL)
        {
            if(!tree->lower_than(aux->pair->key, key) && !tree->lower_than(key, aux->pair->key)){
                tree->current = aux;
                return aux->pair;
            }
...
void insertTreeMap(TreeMap * tree, void* key, void * value) {
    ...
    while(aux != NULL)
        {
            padre = aux;
            if(!tree->lower_than(aux->pair->key, key) && !tree->lower_than(key, aux->pair->key)) return;
            if(tree->lower_than(aux->pair->key, key) == 1) aux = aux->right;
            else aux = aux->left;
        }
    new_node->parent = padre;
    if(padre == NULL) tree->root = new_node;

    else if(tree->lower_than(key, padre->pair->key)) padre->left = new_node;
...
void removeNode(TreeMap * tree, TreeNode* node) {
    ...
    else
    {
        ...
        TreeNode* hijo = min->right;
        if(min->parent->left == min) min->parent->left = hijo;
